...
function formatJSON(data) {
	...
	for(var i = 0; i < jsonFeed.length; i++) {
		...
		output = output + "<div class='trianges top-triangle'></div>";
		output = output + "<div class='trianges bottom-triangle'></div>";

		output = output + "<div class='content'>";
		output = output + "<div class='show-more' id='item" + i + "' onclick='javascript:itemSelected(" + i + ")'></div>"; // Show extra info
		output = output + "<div class='plus-button' onclick='javascript:itemSelected(" + i + ")'><i class='material-icons'>add</i></div>"; // plus icon
		output = output + "<div class='title'>" + jsonFeed[i].name.replace(/ /g,"</br>") + "</div>"; // Bar Name
	...
	var contents = document.getElementById("list").innerHTML = output; // places output into vew variable
	var format = contents.replace(/\n/gi, "<br>"); // replaces \n in json file with line breaks
	$("#list").html(format); // puts new output into container div
	positionPlusButton();


	//$(".background-triangle").next(".background-square").attr( "style", "top: " + ($(".background-triangle").position().top + ($(".background-triangle").outerHeight()) - 1) +  ";")
	//$("iframe").prev(".logo").attr( "style", "top: " + ($("iframe").position().top - ($(".logo").outerHeight() + 30)) + ";")
...
function itemSelected(i) {
	...
	output = output + "<div class='background-square'></div>";
...
function closeDetails() {
	...
	$(".trianges").stop().fadeIn("fast");

}

function positionPlusButton() {
	$( ".list-item .show-more" ).each(function() {
		var showPosition = $(this).position().top;
		var newPosition = showPosition + ($(this).outerHeight()*0.5);		
		var button = $(this).next(".plus-button");
		var buttonHeight = button.outerHeight();
		button.attr( "style", "top: " + (newPosition - buttonHeight*0.5) + ";")
	});
}
